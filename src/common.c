...
int accept_udp(int listener){
    ...
    DBG(BLUE"<SYN recived>"NONE"%s:%d\n", inet_ntoa(client.sin_addr), ntohs(client.sin_port));
    msg.flag = UDP_SYN_ACK;
    if ((sockfd = socket_create_udp(9001)) < 0) {
        return -1;
    }
    int retval = connect(sockfd, (struct sockaddr *)&client, len);
    // UDP 中调用 connect 用于绑定 ip&port， 并且可以多次绑定， 每次绑定都会使上一次的绑定解除， 然后绑定新节点
    // 一定程度上可以加快效率
    // 使用 connect 绑定之后 可以使用 send recv 函数  
    // 未使用connect 则只能使用sendto  和recvfrom  函数
    if (retval < 0) {
...
int get_line(int cfd, char *buf, int size) {
    ...
    while ((i < size - 1) && (c != '\n')) {
        ...
        if (n > 0) {
            if (c == '\r') {
                n = recv(cfd, &c, 1, MSG_PEEK); // 试探性 的读取数据， 读完， 数据不会消失
                if (n > 0 && (c == '\n')) {
                    recv(cfd, &c, 1, 0);
                }else {
                    c = '\n';
